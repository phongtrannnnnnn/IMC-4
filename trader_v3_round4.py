...
class Trader:
    ...
    VE_AR_COEFF = -0.16
    VE_SPREAD_HALF = 2         # market spread ~5, half = 2.5
    VE_TAKE_THRESHOLD = 3.0    # wider: only take clear mispricings
    VE_SKEW_COEFF = 0.08       # strong skew to prevent directional buildup
    VE_BASE_SIZE = 15           # small size — tight spread = adverse selection risk
    VE_COOLDOWN = 200
    VE_CP_BIAS = 1.5
    ...
    def _ve(self, od, pos, mid, ts, s, cp, lim):
        """VE: Track prices only. Market-making unprofitable in 5-tick spread."""
        orders = []

        # Track prices for VEV vol estimation
        p = s.setdefault("ve_p", [])
        p.append(mid)
        if len(p) > self.WINDOW:
            p[:] = p[-self.WINDOW:]

        # No active trading — adverse selection kills all edge in this spread
