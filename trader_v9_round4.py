...
# VEV_5000 stays here: BS path gets 5x more flow than oracle (v8 lesson)
...
class Trader:
    ...
    def _hp(self, state, fair, cp, ts, s):
        ...
        # v9 EOD: gradual flatten from t=90k
        # v7 abrupt dump at t=98.5k cost -1,261
        # v8 t=80k start was too early (80k-90k still profitable)
        if ts > 90000:
            if abs(pos) > 5:
                fq = min(abs(pos), 15)
                if pos > 0:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  VELVETFRUIT_EXTRACT — raw wmid ±2 (EMA was -128 on official)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def _vev_atm(self, sym, od, pos, ve_fair, strike, lim, tte, ts):
        orders: List[Order] = []
        MAX = min(200, lim)
        pos_reserve = 30  # v9: revert to 30 (v8's 60 cost -2.3k in revenue)
